...
public class SaveUserWithConflictStatusTest {
    @Autowired
    ...
    public void before() throws Exception{
        ...
        ConfirmationTokenModel confirmationTokenModel =  confirmTokenService.findByUsername("mohit@itmusketeers.com");
        requestBuilder = MockMvcRequestBuilders
                            .get("/user/confirm-account")
                            .param("token", confirmationTokenModel.getConfirmationToken());
    ...
    @Test
    public void testSaveUserWithConflictStatus() throws Exception {
        RequestBuilder requestBuilder = MockMvcRequestBuilders
                            .post("/user/save")
                            .accept(MediaType.APPLICATION_JSON).content(exampleUserJson)
                            .contentType(MediaType.APPLICATION_JSON);
        MvcResult result = mockMvc.perform(requestBuilder).andReturn();
        MockHttpServletResponse response = result.getResponse();
        assertThat(response.getStatus(), anyOf(is(HttpStatus.CONFLICT.value())));

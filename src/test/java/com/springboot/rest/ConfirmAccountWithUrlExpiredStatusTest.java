...
public class ConfirmAccountWithUrlExpiredStatusTest {
    @Autowired
    ...
    public void before() throws Exception{
        ...
        confirmTokenService.deleteConfirmationTokenByUsername("mohit@itmusketeers.com");
        confirmationTokenModel=confirmTokenService.findByUsername("mohit@itmusketeers.com");
    }
    
    @Test
    public void testConfirmAccountWithUrlExpiredStatus()throws Exception{
        RequestBuilder requestBuilder = MockMvcRequestBuilders
                            .get("/user/confirm-account")
                            .param("token", confirmationTokenModel!=null ?  confirmationTokenModel.getConfirmationToken() : "");
        MvcResult result = mockMvc.perform(requestBuilder).andReturn();
        MockHttpServletResponse response = result.getResponse();
        assertThat(response.getStatus(), anyOf(is(HttpStatus.INTERNAL_SERVER_ERROR.value())));
    }
}
